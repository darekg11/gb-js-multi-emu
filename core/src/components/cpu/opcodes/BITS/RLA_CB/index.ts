...
const handle = (payload: IOpCodeHanlePayload): number => {
    const newCarryFlag = payload.CPU.getRegisterAValue() > 0x7F;
    payload.CPU.setRegisterAValue((payload.CPU.getRegisterAValue() << 1 & 0xFF) | (payload.CPU.isCarryFlagSet() ? 1 : 0));
    if (newCarryFlag) {
