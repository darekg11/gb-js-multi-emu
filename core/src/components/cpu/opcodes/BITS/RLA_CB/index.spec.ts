...
describe("RLA", () => {
    ...
    test("Should rotate register A value to the left by 1 bit. Should unset carry flag if bit 7 of register A is not set. Unset carry flag set", () => {
        ...
        expect(cpu.getRegisterDEValue()).toBe(0);
        expect(cpu.getRegisterHLValue()).toBe(0);
        expect(cpu.getRegisterAFValue()).toBe(EXPETCTED_REG_A_VALUE << 8);
        expect(cpu.getProgramCounter()).toBe(2);
        expect(cpu.isZeroFlagSet()).toBe(false);
        expect(cpu.isSubtractionFlagSet()).toBe(false);
        expect(cpu.isHalfCarryFlagSet()).toBe(false);
    ...
    test("Should rotate register A value to the left by 1 bit. Should unset Zero Flag is result is not 0", () => {
        const REG_A_VALUE = 32;
        const EXPETCTED_REG_A_VALUE = 64;
        const cpu = new CPU();
        const memory = new Memory(new EventBus);
