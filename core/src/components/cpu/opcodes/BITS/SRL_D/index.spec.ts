...
describe("SRL_D", () => {
    test("Should rotate register D value to the right by 1 bit. Should set carry flag if bit 0 of register D is set. Should unset zero flag if result is not zero.", () => {
    ...
    test("Should rotate register D value to the right by 1 bit. Should unset carry flag if bit 0 of register D is not set. Should set zero flag if result is zero.", () => {
        const REG_D_VALUE = 0;
        const EXPETCTED_REG_D_VALUE = 0;
        const cpu = new CPU();
        const memory = new Memory(new EventBus);
