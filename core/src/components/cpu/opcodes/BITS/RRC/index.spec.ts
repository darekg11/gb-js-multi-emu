...
describe("RRC", () => {
    test("Should rotate register C value to the right by 1 bit. Should set carry flag if bit 0 of register C is set. Carry flag set", () => {
        const REG_C_VALUE = 5;
        // >> 1 + 1 from carry flag in bit 0 & 255
        // Carry flag makes it 0x80 | >> 1
        const EXPETCTED_REG_C_VALUE = 130;
        const cpu = new CPU();
    ...
    test("Should rotate register C value to the right by 1 bit. Should unset carry flag if bit 0 of register C is not set. Unset carry flag set", () => {
        const REG_C_VALUE = 4;
        // >> 1 + 0 from carry flag in bit 0 & 255
        const EXPETCTED_REG_C_VALUE = 2;
        const cpu = new CPU();
        const memory = new Memory(new EventBus);
    ...
    test("Should rotate register C value to the right by 1 bit. Should unset Zero Flag is result is not 0", () => {
        const REG_C_VALUE = 4;
        const EXPETCTED_REG_C_VALUE = 2;
        const cpu = new CPU();
        const memory = new Memory(new EventBus);
