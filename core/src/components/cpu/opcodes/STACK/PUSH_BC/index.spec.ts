...
describe("PUSH_BC", () => {
    test("Should store B and C registers on stack, decreases SP Register by 2, increase PC by 1", () => {
        const cpu = new CPU();
        ...
        expect(memory.read8BitsValue(SP_INDEX - 1)).toBe(REG_B_VALUE);
        expect(memory.read8BitsValue(SP_INDEX - 2)).toBe(REG_C_VALUE);
    });
