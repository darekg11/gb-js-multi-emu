...
const handle = (payload: IOpCodeHanlePayload): number => {
    const newCarryFlag = (payload.CPU.getRegisterLValue() & 0x01) === 1;
    payload.CPU.setRegisterLValue((payload.CPU.isCarryFlagSet() ? 0x80 : 0) | (payload.CPU.getRegisterLValue() >> 1) & 0xFF);
    if (newCarryFlag) {
