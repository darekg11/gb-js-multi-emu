...
describe("OR_A_VALUE_OF_MEMORY_CELL_FROM_HL_REGISTER", () => {
    test("Should bitwise OR register A and memory value from memory index under HL register, store result in register A. Do not set ZERO flag.", () => {
        const cpu = new CPU();
    ...
    test("Should bitwise OR register A and memory value from memory index under HL register, store result in register A. Do SET ZERO flag.", () => {
        const cpu = new CPU();
        const memory = new Memory(new EventBus);
        const REG_A = 0;
        const INDEX = 40401;
