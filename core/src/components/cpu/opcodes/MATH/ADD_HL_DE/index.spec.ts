...
describe("ADD_HL_DE", () => {
    test("Should add value of reg DE to reg HL, increase PC by 1 and Subtraction flag to 0", () => {
        const cpu = new CPU();
        const memory = new Memory(new EventBus);
        const REG_HL = 2;
        const REG_DE = 10;
        const RESULT = REG_HL + REG_DE;
        ...
        expect(cpu.getRegisterBValue()).toBe(0);
        expect(cpu.getRegisterCValue()).toBe(0);
        expect(cpu.getRegisterDValue()).toBe(REG_DE >> 8);
        expect(cpu.getRegisterEValue()).toBe(REG_DE & 0x00FF);
        expect(cpu.getRegisterFValue()).toBe(0);
        expect(cpu.getRegisterHValue()).toBe(RESULT >> 8);
        expect(cpu.getRegisterLValue()).toBe(RESULT & 0x00FF);
        expect(cpu.getRegisterBCValue()).toBe(0);
        expect(cpu.getRegisterDEValue()).toBe(REG_DE);
