...
const handle = (payload: IOpCodeHanlePayload) => {
    const newCarryFlag = (payload.CPU.getRegisterEValue() & 0x01) === 1;
    payload.CPU.setRegisterEValue((payload.CPU.isCarryFlagSet() ? 0x80 : 0) | (payload.CPU.getRegisterEValue() >> 1) & 0xFF);
    if (newCarryFlag) {
