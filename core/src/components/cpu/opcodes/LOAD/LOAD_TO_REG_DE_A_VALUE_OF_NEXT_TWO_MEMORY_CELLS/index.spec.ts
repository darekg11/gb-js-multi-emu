...
describe("LOAD_TO_REG_DE_A_VALUE_OF_NEXT_TWO_MEMORY_CELLS", () => {
    test("Should set value correctly and increase PC by 3", () => {
        const TEST_VALUE_FIRST_HALF = 0xDD;
        const TEST_VALUE_SECOND_HALF = 0xEA;
        ...
        expect(cpu.getRegisterBValue()).toBe(0);
        expect(cpu.getRegisterCValue()).toBe(0);
        expect(cpu.getRegisterDValue()).toBe(TEST_VALUE_SECOND_HALF);
        expect(cpu.getRegisterEValue()).toBe(TEST_VALUE_FIRST_HALF);
        expect(cpu.getRegisterFValue()).toBe(0);
        expect(cpu.getRegisterHValue()).toBe(0);
        expect(cpu.getRegisterLValue()).toBe(0);
        expect(cpu.getRegisterBCValue()).toBe(0);
