...
describe("SET_7_H", () => {
    test("Should set bit 7 of register H to 1.", () => {
        const REG_H_VALUE = 0;
        const EXPECTED_REG_H_VALUE = 128;
        const cpu = new CPU();
        ...
        expect(cpu.getRegisterHValue()).toBe(EXPECTED_REG_H_VALUE);
        expect(cpu.getRegisterLValue()).toBe(0);
        expect(cpu.getRegisterBCValue()).toBe(0);
        expect(cpu.getRegisterDEValue()).toBe(0);
        expect(cpu.getRegisterHLValue()).toBe(EXPECTED_REG_H_VALUE << 8);
